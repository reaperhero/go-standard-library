...
func Test_Rand__02(t *testing.T) {
	random.New().String(10)          // 随机字符串
	fmt.Println(GetRandomString(10))     // 随机字符串
	fmt.Println(DigitCheckcode(10))  // 随机数字
...
// DigitCheckcode ...
func DigitCheckcode(length int) string {
	str := "0123456789"
	result := []byte{}
	r := rand.New(rand.NewSource(time.Now().UnixNano()))
	for i := 0; i < length; i++ {
